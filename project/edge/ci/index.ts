...
new Elysia()
    ...
    .post("/workflow", async ({ headers, body, store }) => {
        ...
        if (name === WORKFLOW_NAME && conclusion === "success") {
            ...
            }).then(res => res.json() as object).catch(err => {
            ...
            if (!foundArtifact)
                return { status: "artifact not found" };
            ...
            return await new Promise((resolve, reject) => {
                ...
                Readable.fromWeb(res.body as ReadableStream).pipe(fsync.createWriteStream(path)).on("finish", () => {
